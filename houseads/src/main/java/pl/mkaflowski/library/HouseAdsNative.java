    ...
    private void setUp(String response) {
        ...
        try {
            ...
            for (int object = 0; object < array.length(); object++) {
                ...
                if (hideIfAppInstalled && HouseAdsHelper.isAppInstalled(mContext, jsonObject.optString("app_uri")))
                    array.remove(object);
        ...
        if (val.size() > 0) {
            ...
            if (price != null && !dialogModal.getPrice().trim().equals(""))
                price.setText(String.format("Price: %s", dialogModal.getPrice()));
            if (ratings != null && dialogModal.getRating() != 0)
